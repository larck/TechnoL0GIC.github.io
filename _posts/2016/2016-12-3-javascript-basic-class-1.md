...
저는 ***Learning By Doing*** 이란 단어를 참 좋아합니다. 말로 지루한 이론을 듣기만 하고 직접 체험해보지 않으면, 절대 자기것으로 될 수 없다 생각합니다. 우리말로 바꾸자면 ***백문이 불여일견*** 과 일맥상통 하겠군요. 

준비물이 필요한데, 저희는 node.js로 서버 개발을 하려는 것도 아니고, React Native로 모바일 앱 개발을 하려는 것도 아니고, 단순히 웹브라우저에서 작동하는 자바스크립트를 작성하려 하기 때문에 텍스트 에디터와 웹브라우저만 준비해주시면 됩니다. 어느것을 사용하셔도 상관 없으나 저는 텍스트 에디터로는 [Atom](http://atom.io)를 웹브라우저로는 [Chrome](http://chrome.google.com)을 추천해드립니다.

*(이 과정이 귀찮으신 분들은 [JSfiddle](https://jsfiddle.net/)을 사용하도록 합시다!)*
...
![](/images/javascript-basic-1-1.png)
